...
- **Bug found during on-device testing:** the playlist-position label ("`4/116 • ...`") showed the raw filename (e.g. "01 - Fireflies") instead of the ID3 tag title (e.g. "Fireflies") once large playlists finished loading. Root cause: this label has always read `PlaylistTrack.title`, which is set once at M3U-parse time from `#EXTINF` (if present) or the filename — it was never derived from ID3 tags; the gap was just never visible before because a playlist this size previously crashed before reaching playback. Fixed in `PlaybackService.loadTrack` by patching the current playlist entry's title in place with the real ID3 tag once it's read for the track that just loaded (reusing the metadata read already done for the header, not an extra per-track tag scan), so the playlist bar now matches the header.
